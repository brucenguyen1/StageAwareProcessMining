...
public class SPFMultipleTISView extends NodeView {
	public SPFMultipleTISView(SPF bpf, String desc) {
		super(bpf, null, desc);
	}

	public XYDataset createDataset(SPFStage stage) {
	...
	public XYDataset createTotalDataset() {
	...
	public JFreeChart createChart(XYDataset xydataset) {
		...
		for (SPFStage stage : bpf.getStages()) {
			datasetList.add(this.createDataset(stage));
		}
		datasetList.add(createTotalDataset());
