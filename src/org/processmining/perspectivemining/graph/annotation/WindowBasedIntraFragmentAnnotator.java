...
import org.processmining.perspectivemining.graph.model.Fragment;
...
public class WindowBasedIntraFragmentAnnotator extends WindowBasedAnnotator {
...
//			Map<ObjectInstance, Fragment> mapObjectFragment = this.getSameObjectFragments(windowFragments);
			for (Fragment fragment : windowFragments) {
				...
				for (EventRow event : fragment) {
					...
					if (!pGraph.getSetting().isNode1Node2SameSchema()) {
						ProcessObject object2 = ObjectUtils.project(pConfig.getNodeSchema2(), event);
						
						Node node2 = pGraph.getNode(object2.getValueString()); //create node if not exists
		...
		// Just call this once for one window
