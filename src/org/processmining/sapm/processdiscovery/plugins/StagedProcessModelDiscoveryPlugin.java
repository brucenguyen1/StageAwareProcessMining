...
import org.processmining.sapm.processdiscovery.MCAS;
...
public class StagedProcessModelDiscoveryPlugin {
	...
	public Object[] mine(final UIPluginContext context, XLog log, StageDecomposition stageModel) {
		...
		// row #3
		JCheckBox chkAdjust = SlickerFactory.instance().createCheckBox("Run adjusting step?", true);
		panel.add(chkAdjust,"1,3");
		
		// row #4
		JCheckBox chkSoundness = SlickerFactory.instance().createCheckBox("Select sound models?", false);
		panel.add(chkSoundness,"1,4");
		...
		MCAS miner = new MCAS(	1.0*imprecisionSlider.getValue()/100, 
								...
								true, 
								true,
								chkAdjust.isSelected(),
								chkSoundness.isSelected(),
								baseMiner);
		try {
			MiningResult mresult = miner.mineBestModel(log, ModelMetrics.FSCORE, stageModel);
