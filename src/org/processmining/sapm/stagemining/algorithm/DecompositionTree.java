...
import org.processmining.sapm.stagemining.graph.Vertex2;
...
//    @Deprecated
//    public List<String> getTransitionActivities(int levelIndex) throws Exception {
//    	List<String> transitions = new ArrayList<String>();
//    	for (IVertex v : this.getTransitionNodes(levelIndex)) {
//    		transitions.add(v.getName());
//    	}
//    	return transitions;
//    }
...
//					if (v != this.graph.getSource() && v != this.graph.getSink()) { //remove the fake start/end node
//						stageNodeNames.add(v.getName().toLowerCase());
//					}
//				}
//				if (i < transitionNodes.size()) stageNodeNames.add(transitionNodes.get(i).getName().toLowerCase());
...
//	}
	...
	/**
	 * Return ActivityClusterArray for use with the Divide and Conquer framework in ProM
	 * @param levelIndex
	 * @param log: original log
	 * @return ActivityClusterArray
	 * @throws Exception
	 */
//	public ActivityClusterArray getClusters(int levelIndex, XLog log) throws Exception {
//		ActivityClusterArray clusters = ActivityClusterArrayFactory.createActivityClusterArray();
//		List<Set<String>> activityLabels = this.getActivityLabelSets(levelIndex);
...
//		for (Set<String> labelCluster : activityLabels) {
//			Set<XEventClass> cluster = new HashSet<>();
//			for (String activityLabel : labelCluster) {
//				XEventClass ec = eventClasses.getByIdentity(activityLabel);
//				if (ec!=null) {
//					cluster.add(ec);
//				}
//				else {
//					throw new Exception("Cannot find event class for activity label='"+activityLabel+"' on the decomposition tree.");
//				}
//			}
