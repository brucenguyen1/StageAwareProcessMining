package org.processmining.sapm.stagemining.graph;
...
import org.processmining.sapm.stagemining.vis.StageNode;
import org.processmining.sapm.utils.ConnectedComponentSearch;
...
//	private XLog log = null;
    ...
    public SortedSet<Vertex2> searchCutPoints(WeightedDirectedGraph filteredGraph) throws Exception {
 		//-------------------------------
...
// 		GraphUtils.printMatrix(bidiAdjacentMatrix);
 		...
 		for (IVertex v : this.getVertices()) {
 			...
 			// Don't cut on exit node: the true cut-point is not an exit node
// 			if (!this.getEdgesWithSourceAndTarget((Vertex)v, (Vertex)this.getSink()).isEmpty()) {
// 				continue;
// 			}
 			...
 			Double degree = filteredGraph.getUnweightedNodeDegree(v2);
// 			System.out.println("Vertex " + v.getName() + ": Min-cut=" + minCut + ", Cut-set: " + cutSet.toString());
 			
 			v2.setMinCut(minCut);
 			v2.setMinCutSet(cutSet);
 			
 			if (minCut < this.getTotalFlow() && degree >= 2) sortedVertices.add(v2);
    ...
    public List<Set<IVertex>> cut(Vertex2 cutPoint) throws Exception {
		...
		this.removeEdges(cutPoint.getMinCutSet()); //this.removeEdges(cutPoint.getMinCutSet());
...
//		}
    ...
    public double getUnweightedNodeDegree(Vertex v) {
    	double degreeIn = this.getEdgesWithTarget(v).size();
    	double degreeOut = this.getEdgesWithSource(v).size();
    	return Math.max(degreeIn, degreeOut);
    }
...
//    					target != this.getSink()) {
    ...
    public double getWeight(Vertex source, Vertex target) {
    	WeightedDirectedEdge<IVertex> edge = this.getDirectedEdge(source, target);
    	if (edge != null) {
    ...
    public WeightedDirectedGraph clone() {
    	WeightedDirectedGraph newGraph = new WeightedDirectedGraph();
    	for (IVertex v : this.getVertices()) {
    		newGraph.addVertex((Vertex2)v);
    	}
    	for (WeightedDirectedEdge<IVertex> e : this.getEdges()) {
    		newGraph.addEdge(e.getSource(), e.getTarget(), e.getWeight());
    	}
    	newGraph.setSource((Vertex)this.source);
    	newGraph.setSink((Vertex)this.sink);
    	return newGraph;
    }
    
    /*
     * Convert from this graph to a StageGraph
     */
    public StageGraph getStageGraph() {
    	StageGraph stageGraph = new StageGraph(this.getVertices().size());
    	Map<Vertex,StageNode> nodeMap = new HashMap<>();
    	int nodeIndex = 0;
    	double totalWeight = this.getTotalFlow();
    
    	for (Vertex v : this.getVertices()) {
    		StageNode node = new StageNode(stageGraph, nodeIndex, "<html>" + v.getName() + "</html>", 3);
    		stageGraph.addNode(node, nodeIndex);
			nodeMap.put(v, node);
			nodeIndex++;
    	}
    	
    	for (WeightedDirectedEdge<IVertex> e : this.getEdges()) {
    		stageGraph.addEdge(nodeMap.get(e.getSource()), nodeMap.get(e.getTarget()), e.getWeight(), 1.0*e.getWeight()/totalWeight);
    	}
