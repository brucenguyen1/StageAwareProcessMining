...
import org.processmining.sapm.performancemining.util.Logger;
...
public class StageBasedEnhancementChecker {
	...
	public StageBasedEnhancementChecker(XLog log, SPFConfig config) {
		...
		Logger.println("Log information: " + LogUtils.getValue(log.getAttributes().get("name")));
	...
	private void selectModelStageList(XLog log) {
		...
		for (XTrace trace : log) {
			...
			for (XEvent event : trace) {
				String stage = LogUtils.getValue(event.getAttributes().get("stage")).toString().toLowerCase();
	...
	private boolean checkAttributes(XLog log) {
		...
		for (XTrace trace : log) {
			...
			if (!trace.getAttributes().containsKey("status")) {
				Logger.println("Trace " + LogUtils.getConceptName(trace) + ": missing 'status' attribute.");
				error = true;
			}

			for (XEvent event : trace) {
				if (!event.getAttributes().containsKey("stage")) {
					Logger.println("Trace " + LogUtils.getConceptName(trace) + ". Event "
							+ LogUtils.getConceptName(event) + ": missing 'stage' attribute.");
					error = true;
				}
			}
	...
	private boolean checkStageBasedEnhancementConditions() {
		...
		for (int i = 0; i < alltraceStageList.size(); i++) {
			String status = LogUtils.getValue(log.get(i).getAttributes().get("status")).toString().toLowerCase();
			for (int j = 0; j < modelStageList.size(); j++) {
				if ((stageTable[i][j] == 0) && (j < modelStageList.size() - 1) && (stageTable[i][j + 1] != 0)) {
					Logger.println("Trace " + LogUtils.getConceptName(log.get(i)) + ": unfound stage + '"
							+ modelStageList.get(j) + "'");
					result = false;
				}
				if ((stageTable[i][j] == 0) && (status.equals("complete") || status.equals("completed"))) {
					Logger.println("Trace " + LogUtils.getConceptName(log.get(i))
							+ ": complete case with missing stage + '" + modelStageList.get(j) + "'");
					result = false;
				}
				if (stageTable[i][j] == -1) {
					Logger.println("Trace " + LogUtils.getConceptName(log.get(i)) + ": wrong order at stage + '"
							+ modelStageList.get(j) + "'");
					result = false;
				}
			}
	...
	private boolean checkStartCompleteEvents(XLog log) {
		...
		for (XTrace trace : log) {
			...
			for (XEvent event : trace) {
				String stage = LogUtils.getValue(event.getAttributes().get("stage")).toString().toLowerCase();
				String actName = LogUtils.getConceptName(event);
					...
					if (!previousStage.equals("")) {
						for (String eventName : startEventCountMap.keySet()) {
							if (!completeEventCountMap.containsKey(eventName)
								...
								Logger.println("Trace " + LogUtils.getConceptName(trace)
				...
				String transition = LogUtils.getLifecycleTransition(event).toLowerCase();
