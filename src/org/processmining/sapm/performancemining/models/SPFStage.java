 ...
 * @author Bruce Nguyen
 */
public class SPFStage {
	private final List<SPFCell> flowCells = new ArrayList<SPFCell>();
	...
	private SPFStage actualStage = null;

	public SPFStage(String name, SPF bpf) {
		this.name = name;
		this.bpf = bpf;
	}
	...
	public SPFStage getActualStage() {
		return this.actualStage;
	}
	
	public void setActualStage(SPFStage actualStage) {
	...
	public void computeFlowCellCharacteristics(int index) {
		...
		SPFCell cell = new SPFCell(timePoint);
	...
	private int searchDepartingPointAtQueue(int index) {
		...
		if (foundIndex == -1) {
			foundIndex = bpf.getTimeSeries().size()-1;
		}
		return foundIndex;
	}
	...
	public void computeCharacteristics() throws Exception {
		...
		for (int i = 0; i < bpf.getTimeSeries().size(); i++) {
			counter++;
			if (counter % 1000 == 0) {
				bpf.getConfig().getContext().getProgress().inc();
			}
			computeFlowCellCharacteristics(i);
		}
	}

	public List<SPFCell> getFlowCells() {
		return flowCells;
	}
	 ...
	 * @return: StageBlock
	 */
	public SPFCell getFlowCellByTime(long timestamp) {
