...
import org.processmining.sapm.performancemining.ui.jidesoft.RangeSlider;
...
public class SPFComparisonView extends NodeView implements ActionListener {
	...
	public SPFComparisonView(SPF bpf, String label) {
	...
	public JPanel createPanel() {
		...
		//		JLabel labelView = SlickerFactory.instance().createLabel("<html><center>" + 
		//																"<h2>" + this.getLabel() + "</h2>" +
		//																"</center></html>");
		//		labelView.setHorizontalAlignment(JLabel.CENTER);
		//		labelView.setVerticalAlignment(JLabel.CENTER);
		//		mainPanel.add(labelView, "1,2");
	...
	private void updateLabelRange(DateTime start, DateTime end) {
		labelRange.setText("<html><center>"
				...
				+ end.withZone(DateTimeZone.forTimeZone(bpf.getConfig().getTimeZone())).toString(
						"EEE, d MMM yyyy HH:mm:ss Z") + " (" + Days.daysBetween(start, end).getDays() + " days)</h4>"
				+ "</center></html>");
	...
	private JPanel createTablePanel(DateTime start, DateTime end) {
		...
		SPF fullbpf;
		try {
			fullbpf = SPFManager.getInstance().get(0);

			tableContent[0][0] = "System";
			tableContent[0][1] = formatter.format(100 * bpf.getMeanArrivalRate(start, end)
					/ fullbpf.getMeanArrivalRate(start, end))
					+ "%";
			tableContent[0][2] = formatter.format(100 * bpf.getMeanDepartureRate(start, end)
					/ fullbpf.getMeanDepartureRate(start, end))
					+ "%";
			tableContent[0][3] = formatter.format(100 * bpf.getMeanExitRate(start, end)
					/ fullbpf.getMeanExitRate(start, end))
					+ "%";
			tableContent[0][4] = formatter.format(100 * bpf.getMeanWIP(start, end) / fullbpf.getMeanWIP(start, end))
					+ "%";
			tableContent[0][5] = formatter.format(100 * bpf.getMeanTIS(start, end) / fullbpf.getMeanTIS(start, end))
					+ "%";

			if (bpf.getConfig().getCheckStartCompleteEvents()) {
				if (fullbpf.getMeanFE(start, end) != 0) {
					tableContent[0][6] = formatter.format(100 * bpf.getMeanFE(start, end)
							/ fullbpf.getMeanFE(start, end))
							+ "%";
				} else {
					tableContent[0][6] = "--";
				}
			...
			for (SPFStage stage : bpf.getStages()) {
				SPFStage fullstage = fullbpf.getStageByName(stage.getName());
				tableContent[i][0] = stage.getName() + "-queue";

				tableContent[i][1] = formatter.format(100 * stage.getMean(SPF.CHAR_QUEUE_ARRIVAL_RATE, start, end)
						/ fullstage.getMean(SPF.CHAR_QUEUE_ARRIVAL_RATE, start, end))
						+ "%";
				tableContent[i][2] = formatter.format(100 * stage.getMean(SPF.CHAR_QUEUE_DEPARTURE_RATE, start, end)
						/ fullstage.getMean(SPF.CHAR_QUEUE_DEPARTURE_RATE, start, end))
						+ "%";
				tableContent[i][4] = formatter.format(100 * stage.getMean(SPF.CHAR_QUEUE_CIP, start, end)
						/ fullstage.getMean(SPF.CHAR_QUEUE_CIP, start, end))
						+ "%";
				tableContent[i][5] = formatter.format(100 * stage.getMean(SPF.CHAR_QUEUE_TIS, start, end)
						/ fullstage.getMean(SPF.CHAR_QUEUE_TIS, start, end))
						+ "%";

				tableContent[i + 1][0] = stage.getName() + "-service";
				tableContent[i + 1][1] = formatter.format(100
						* stage.getMean(SPF.CHAR_SERVICE_ARRIVAL_RATE, start, end)
						/ fullstage.getMean(SPF.CHAR_SERVICE_ARRIVAL_RATE, start, end))
						+ "%";
				tableContent[i + 1][2] = formatter.format(100
						* stage.getMean(SPF.CHAR_SERVICE_DEPARTURE_RATE, start, end)
						/ fullstage.getMean(SPF.CHAR_SERVICE_DEPARTURE_RATE, start, end))
						+ "%";
				tableContent[i + 1][3] = formatter.format(100 * stage.getMean(SPF.CHAR_SERVICE_EXIT_RATE, start, end)
						/ fullstage.getMean(SPF.CHAR_SERVICE_EXIT_RATE, start, end))
						+ "%";
				tableContent[i + 1][4] = formatter.format(100 * stage.getMean(SPF.CHAR_SERVICE_CIP, start, end)
						/ fullstage.getMean(SPF.CHAR_SERVICE_CIP, start, end))
						+ "%";
				tableContent[i + 1][5] = formatter.format(100 * stage.getMean(SPF.CHAR_SERVICE_TIS, start, end)
						/ fullstage.getMean(SPF.CHAR_SERVICE_TIS, start, end))
						+ "%";

				if (bpf.getConfig().getCheckStartCompleteEvents()) {
					tableContent[i + 1][6] = formatter.format(100
							* stage.getMean(SPF.CHAR_SERVICE_FLOW_EFFICIENCY, start, end)
							/ fullstage.getMean(SPF.CHAR_SERVICE_FLOW_EFFICIENCY, start, end))
							+ "%";
				} else {
					tableContent[i + 1][6] = "--";
				}
	...
	private JPanel createRangePanel(DateTime start, DateTime end) {
		...
		minField.setText(start.toString("dd.MM.yyyy HH:mm"));
		maxField.setText(end.toString("dd.MM.yyyy HH:mm"));
