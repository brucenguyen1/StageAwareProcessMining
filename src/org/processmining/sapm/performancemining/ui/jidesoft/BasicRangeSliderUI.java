...
public class BasicRangeSliderUI extends BasicSliderUI {
	...
	protected class RangeTrackListener extends TrackListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			if ((handle == MOUSE_HANDLE_MAX) || (handle == MOUSE_HANDLE_MIN) || (handle == MOUSE_HANDLE_MIDDLE)
				...
				handleOffset = (slider.getOrientation() == SwingConstants.VERTICAL) ? e.getY()
						- yPositionForValue(((RangeSlider) slider).getLowValue()) : e.getX()
						- xPositionForValue(((RangeSlider) slider).getLowValue());

				mouseStartLocation = (slider.getOrientation() == SwingConstants.VERTICAL) ? e.getY() : e.getX();
		...
		public void mouseDragged(MouseEvent e) {
			...
			int newLocation = (slider.getOrientation() == SwingConstants.VERTICAL) ? e.getY() : e.getX();

			int newValue = (slider.getOrientation() == SwingConstants.VERTICAL) ? valueForYPosition(newLocation)
			...
			switch (handle) {
				...
				case MOUSE_HANDLE_MIDDLE :
					if (((RangeSlider) slider).isRangeDraggable()) {
						int delta = (slider.getOrientation() == SwingConstants.VERTICAL) ? valueForYPosition(newLocation
								- handleOffset)
	...
	protected int getMouseHandle(int x, int y) {
		...
		if (slider.getOrientation() == SwingConstants.VERTICAL) {
			...
			Rectangle midRect = new Rectangle(Math.min(minX, maxX) + thumbRect.width / 2, rect.y, Math.abs(maxX - minX)
	...
	public void scrollByBlock(int direction) {
		synchronized (slider) {
			...
			if (clientProperty == null) {
				...
				((RangeSlider) slider).setLowValue(Math.max(Math.min(oldValue + delta, slider.getMaximum()),
						slider.getMinimum()));
			} else {
				((RangeSlider) slider).setHighValue(Math.max(Math.min(oldValue + delta, slider.getMaximum()),
						slider.getMinimum()));
			}
