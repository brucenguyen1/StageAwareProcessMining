package org.processmining.spm.stagemining.graph;
...
import org.processmining.spm.utils.ConnectedComponentSearch;
...
//    					target != this.getSink()) {
    ...
    public MutableFuzzyGraph getMutableFuzzyGraph() {
    	MutableFuzzyGraph fmGraph = new MutableFuzzyGraph(this.getVertices().size());
    	Map<Vertex,FMNode> nodeMap = new HashMap<>();
    	int nodeIndex = 0;
    	double totalWeight = this.getTotalFlow();
    
    	for (Vertex v : this.getVertices()) {
    		FMNode fmNode = new FMNode(fmGraph, nodeIndex, "<html>" + v.getName() + "</html>", 3);
    		fmGraph.addNode(fmNode, nodeIndex);
			nodeMap.put(v, fmNode);
			nodeIndex++;
    	}
    	
    	for (WeightedDirectedEdge<IVertex> e : this.getEdges()) {
    		fmGraph.addEdge(nodeMap.get(e.getSource()), nodeMap.get(e.getTarget()), 1.0*e.getWeight()/totalWeight, e.getWeight());
    	}
