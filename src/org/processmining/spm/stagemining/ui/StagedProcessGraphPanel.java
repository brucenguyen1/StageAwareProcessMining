...
public class StagedProcessGraphPanel extends JPanel { //, Provider { if it is necessary, we implement Provider
	...
	public StagedProcessGraphPanel(PluginContext context, StagedProcessGraph graph) {
		...
		MutableFuzzyGraph oriGraph = graph.getBaseGraph().getMutableFuzzyGraph();
...
//		if (graph.getStageModel() != null) {
//			MutableFuzzyGraph oriFilteredGraph = graph.getStageModel().getFilteredGraph().getMutableFuzzyGraph();
//			oriFilteredGraph.getAttributeMap().put(AttributeMap.PREF_ORIENTATION, SwingConstants.NORTH);
//			oriFilteredGraphPanel = ProMJGraphVisualizer.instance().visualizeGraph(context, oriFilteredGraph);
//			tabbedPane.addTab("Filtered Graph", oriFilteredGraphPanel);
