...
import org.processmining.spm.stagemining.algorithm.StageMiningExhaustive;
import org.processmining.spm.stagemining.algorithm.StageMiningHighestModularity;
import org.processmining.spm.stagemining.algorithm.StageMiningLowestMinCut;
...
public class StageMiningPlugin {
	...
	public StageDecomposition mineSP(final UIPluginContext context, XLog log) {
		...
		StageDecomposition model=null;
		try {
			model = mineGeneric(context, log, (String)cboAlgo.getSelectedItem(), 
								(Integer)cboStageSize.getSelectedItem(), (String)cboLogs.getSelectedItem());
			context.getFutureResult(0).setLabel("Stage Decomposition: " + 
					"Modularity=" + String.format("%.2f", model.getModularity()) + 
					"; Fowlkes-Mallows=" + String.format("%.2f", model.getGroundTruthIndex()));
			return model;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public StageDecomposition mineGeneric(final UIPluginContext context, XLog log, String algo, 
									int minStageSize, String gtClassOption) throws Exception {
		...
		else if (gtClassOption.equals("BPI17")) {
			gtClassName = "org.processmining.spm.stagemining.groundtruth.StageModelBPI2017";
		}
		
		
		StageDecomposition stageModel = new StageDecomposition(log, minStageSize, miningAlgo);
		if (!gtClassName.equals("")) {
		...
		stageModel.writeResult(System.getProperty("user.dir") + "//SPM_" + algo + "_" + LogUtils.getConceptName(log) + ".txt");
